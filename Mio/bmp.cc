...
bmp::bmp(int32_t width, int32_t height, bool has_alpha) {
    ...
    if (has_alpha) {
        ...
        (*this).bmp_info_header.size_image = (*this).data.size();
    }
}
bmp::~bmp(){}
...
void bmp::grayScale(){
  ...
  for (int32_t y = 0; y < bmp_info_header.height; y++) {
    for (int32_t x = 0; x < bmp_info_header.width; x++) {
      //(*this).data[channels * (yy * bmp_info_header.width + xx) + 0] = data[channels * (y * bmp_info_header.width + x) + 0]* 0.144;
      //(*this).data[channels * (yy * bmp_info_header.width + xx) + 1] = data[channels * (y * bmp_info_header.width + x) + 1]* 0.587;
      //(*this).data[channels * (yy * bmp_info_header.width + xx) + 2] = data[channels * (y * bmp_info_header.width + x) + 2]* 0.299;
        r = (double)(*this).data[channels * (y * bmp_info_header.width + x) + 2];
        g = (double)(*this).data[channels * (y * bmp_info_header.width + x) + 1];
        b = (double)(*this).data[channels * (y * bmp_info_header.width + x) + 0];
        ...
        (*this).data[channels * (y * bmp_info_header.width + x) + 2] = gray;
        (*this).data[channels * (y * bmp_info_header.width + x) + 1] = gray;
        (*this).data[channels * (y * bmp_info_header.width + x) + 0] = gray;
      }
     }
}
void bmp::grayScale2(){
...
void bmp::negative(){
  ...
  for (int32_t y = 0; y < bmp_info_header.height; y++) {
    for (int32_t x = 0; x < bmp_info_header.width; x++) {
        (*this).data[channels * (y * bmp_info_header.width + x) + 2] = L-(*this).data[channels * (y * bmp_info_header.width + x) + 2];
        (*this).data[channels * (y * bmp_info_header.width + x) + 1] = L-(*this).data[channels * (y * bmp_info_header.width + x) + 1];
        (*this).data[channels * (y * bmp_info_header.width + x) + 0] = L-(*this).data[channels * (y * bmp_info_header.width + x) + 0];
      }
     }
}
...
void bmp::copyData(bmp & bmpAux){
    uint32_t channels = bmp_info_header.bit_count / 8;
    for (int32_t y = 0; y < bmpAux.bmp_info_header.height; y++) {
      for (int32_t x = 0; x < bmpAux.bmp_info_header.width; x++) {
        for (int32_t yy = y; yy < bmp_info_header.height; yy=yy+bmpAux.bmp_info_header.height) {
          for (int32_t xx = x; xx < bmp_info_header.width; xx=xx+bmpAux.bmp_info_header.width) {
            (*this).data[channels * (yy * bmp_info_header.width + xx) + 0] = bmpAux.data[channels * (y * bmpAux.bmp_info_header.width + x) + 0];
            (*this).data[channels * (yy * bmp_info_header.width + xx) + 1] = bmpAux.data[channels * (y * bmpAux.bmp_info_header.width + x) + 1];
            (*this).data[channels * (yy * bmp_info_header.width + xx) + 2] = bmpAux.data[channels * (y * bmpAux.bmp_info_header.width + x) + 2];
          }
        }
      }
    }
...
void bmp::setHeight(int32_t & h) {
  ...
  (*this).row_stride = bmp_info_header.height * 3;
